...
class ChatBridgeForwarder:
    ...
    async def start(self):
        ...
        logger.info(
            f"WebSocket服务器运行在 ws://{self.settings['websocket']['host']}:{self.settings['websocket']['port']}"
        )
        logger.info(
            f"ST API服务器运行在 http://{self.settings['st_api']['host']}:{self.settings['st_api']['port']}"
        )
        logger.info(
            f"用户API服务器运行在 http://{self.settings['user_api']['host']}:{self.settings['user_api']['port']}"
        )
    ...
    async def handle_websocket(self, websocket):
        ...
        if token:
            ...
            if client_token != token:
                logger.warning(f"Token验证失败，客户端Token: {client_token}")
                await websocket.close(code=1008, reason="Token验证失败")
                return
            logger.info("Token验证通过")
        else:
            logger.info("Token未设置，跳过验证")
